...
def qa():
	...
	return render_template('qa.html', form=form, question=question, index=index, quizSetId=quizSetId, picture=picture, next=next, username=current_user.name)
...
def manage_quiz():
	...
	return render_template('manage_quiz.html', quizSets=quizSets, questions=show_questions, username=current_user.name)
	...
	if form.validate_on_submit() and id is not None:
		...
		if not (file and allowed_file(file.filename)):
			...
			if quizSet is not None:
				...
				db.session.commit()
				return redirect('/manage_quiz')
	return render_template('edit_question_set.html', form=form, username=current_user.name)
...
def user_change_password():
	...
	return render_template('change_password.html', form=form, username=current_user.name)
...
def user_change_name():
	...
	return render_template('change_name.html', form=form, username=current_user.name)
